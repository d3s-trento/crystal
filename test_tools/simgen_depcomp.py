...
def prepare_binary(simdir, nodes, new_env):
    env = os.environ.copy()
    env.update(new_env)
...
def mk_env(power, channel, sink, n_empty, cca):
    cflags = [
    "-DDEPCOMP18=1",
    "-DLEDS_CONF_ON=0",
    "-DTX_POWER=%d"%power,
    ...
    if testbed == "graz":
        cflags += ["-DPRINT_GRAZ=1"]
        cflags += ["-DDISABLE_DS2411=1"]
    if testbed == "cooja":
        cflags += ["-DCOOJA=1"]
    if testbed in ("indriya", "fbk"):
...
simnum = 0
for (power, channel, sink, n_empty, cca, nodemap) in itertools.product(powers, channels, sinks, n_emptys, ccas, nodemaps):
    n_empty = NemptyTuple(*n_empty)
    cca = CcaTuple(*cca)
    simdir = "sink%03d_p%02d_c%02d_e%.2f_ns%02d_nt%02d_na%02d_ds%02d_dt%02d_da%02d_syna%d_r%02dy%02dz%02dx%02d_dyn%d_cca%d_%d_fe%02d_%s_%s_%s_B%s"%(sink, power, channel, period, n_tx_s, n_tx_t, n_tx_a, dur_s, dur_t, dur_a, sync_ack, n_empty.r, n_empty.y, n_empty.z, n_empty.x, dyn_nempty, cca.dbm, cca.counter, full_epochs, testbed, nodemap, chmap, boot_chop)
    ...
    try:
        ...
        new_env = mk_env(power, channel, sink, n_empty, cca)
        prepare_binary(simdir, all_senders, new_env)
        if nodemap != "all":
